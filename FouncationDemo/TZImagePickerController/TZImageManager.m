...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
    ...
    if ([presets containsObject:AVAssetExportPreset640x480]) {
        if (self.uploadHighQuality) {
            NSLog(@"高清上传");
//             [[NSNotificationCenter defaultCenter] postNotificationName:@"video_url" object:@"uploadHighQuality"];
                ...
                switch (session.status) {
                    ...
                    case AVAssetExportSessionStatusCompleted: {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            [[NSNotificationCenter defaultCenter] postNotificationName:@"video_url" object:outputPath];
                        });
